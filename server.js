...
app.get("/api/reservations", async (req, res) => {
  ...
  const [rows] = await pool.execute(
    `SELECT id, display_name AS name, start_date, end_date, people, equipment, status, message, created_at
     FROM reservations
     ${where}
...
app.get("/api/admin/reservations", requireAdmin, async (_req, res) => {
  const [rows] = await pool.execute(
    `SELECT id, customer_name AS name, phone, start_date, end_date, people, equipment, status, message, created_at
     FROM reservations
...
app.post("/api/reservations", async (req, res) => {
  ...
  if (!name || String(name).trim().length < 2) {
    return res.status(400).json({ error: "예약자 이름을 2글자 이상 입력해주세요." });
  }

  if (!phone || String(phone).trim().length < 8) {
    return res.status(400).json({ error: "연락 가능한 전화번호를 입력해주세요." });
  }

  if (!isDate(startDate) || !isDate(endDate) || startDate > endDate) {
    return res.status(400).json({ error: "예약 시작일과 종료일을 올바르게 선택해주세요." });
  }

  const peopleNumber = Number(people);
  if (!Number.isInteger(peopleNumber) || peopleNumber < 1 || peopleNumber > 30) {
    return res.status(400).json({ error: "인원은 1명부터 30명까지 입력할 수 있습니다." });
  }

  const safeEquipment = Array.isArray(equipment)
    ? equipment.map((item) => String(item).slice(0, 40)).slice(0, 12)
    : [];
  ...
  const [result] = await pool.execute(
    `INSERT INTO reservations
       (customer_name, display_name, phone, start_date, end_date, people, equipment, message, status)
     VALUES
       (:name, :displayName, :phone, :startDate, :endDate, :people, :equipment, :message, 'pending')`,
    {
  ...
  res.status(201).json({
    id: result.insertId,
    status: "pending",
    message: "예약 요청이 접수되었습니다. 관리자가 확인 후 확정합니다."
  });
});
...
app.patch("/api/admin/reservations/:id/status", requireAdmin, async (req, res) => {
  ...
  if (!Number.isInteger(id) || id < 1) {
    return res.status(400).json({ error: "올바른 예약 ID가 아닙니다." });
  }

  if (!["pending", "confirmed", "canceled"].includes(status)) {
    return res.status(400).json({ error: "상태는 pending, confirmed, canceled 중 하나여야 합니다." });
  }
...
app.listen(port, "0.0.0.0", () => {
  console.log(`Camp reservation server listening on port ${port}`);
});
